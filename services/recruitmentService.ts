...
const mapOffer = (r: OfferRow): Offer => ({
  ...
  employmentTypeCustomName: r.employment_type_custom_name || undefined,
  employmentEndDate: r.employment_end_date ? new Date(r.employment_end_date) : undefined,
  supersedesOfferId: r.supersedes_offer_id || undefined,
...
export const saveOffer = async (offer: Partial<Offer>): Promise<Offer> => {
  const payload = {
    ...
    employment_end_date: offer.employmentEndDate ? new Date(offer.employmentEndDate).toISOString().split('T')[0] : null,

...
import {attendanceRequest} from './attendanceDeadline';
...
async function clockRpc<T>(name:string,args:Record<string,unknown>={},write=false):Promise<T>{
 const {data,error}=await attendanceRequest(signal=>supabase.rpc(name,args).abortSignal(signal),write);
 if(error)throw Object.assign(new Error(error.message),{code:error.code});
 return data as T;
}
export const getMyAttendance=()=>clockRpc<AttendanceDay>('get_my_attendance');
export const recordMyAttendance=(action:ClockAction,requestId:string,day:AttendanceDay,evidence?:ClockEvidence)=>clockRpc<AttendanceDay>(evidence?'record_my_attendance_verified':'record_my_attendance',{p_action:action,p_request_id:requestId,p_expected_revision:day.revision,p_work_date:day.workDate,...(evidence?{p_evidence:evidence}:{})},true);
...
export const getMyAttendanceHistory=(week?:string)=>clockRpc<AttendanceHistory>('get_my_attendance_history',{p_week:week||null});

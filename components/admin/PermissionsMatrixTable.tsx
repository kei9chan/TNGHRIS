            ...
            <div className="max-h-[70vh] overflow-auto custom-scrollbar">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border-collapse">
                    <thead className="bg-gray-100 dark:bg-gray-800 sticky top-0 z-10 shadow-sm">
                        <tr>
                            <th scope="col" className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider border-r dark:border-gray-600 bg-gray-100 dark:bg-gray-800 min-w-[150px]">
                                Section / Permission
                            </th>
                            {roles.map(role => (
                                <th key={role} scope="col" className="px-4 py-3 text-center text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider w-48 bg-gray-100 dark:bg-gray-800">
                                    {role}
                                </th>
                            ))}
                        </tr>
                        ...
                        {Object.entries(resourceGroups).map(([groupName, resources]) => (
                            <React.Fragment key={groupName}>
                                <tr className="bg-gray-50 dark:bg-gray-800/50">
                                    <td colSpan={roles.length + 1} className="px-4 py-2 text-sm font-bold text-gray-800 dark:text-gray-200 sticky left-0">
                                        {groupName}
                                    </td>
                                </tr>
                                {(resources as Resource[]).map(resource => (
                                    <tr key={resource} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        <td className="px-4 py-3 font-medium text-sm text-gray-800 dark:text-gray-300 border-r dark:border-gray-600 sticky left-0 bg-white dark:bg-gray-900 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                                            {resource}
                                        </td>
                                        {roles.map(role => (
                                                    ...
                                                    {permissionOrder.map(permission => {
                                                        ...
                                                        return (
                                                            <div key={permission} className="flex flex-col items-center">
                                                                <label htmlFor={id} className="text-xs text-gray-500 select-none cursor-pointer">{permission.charAt(0)}</label>
                                                                <input
                                                                    id={id}
                                                                    type="checkbox"

...
export default function PayrollSubNav(){
 ...
 const groups=payrollGroups.map(g=>({...g,links:links.filter(l=>payrollGroupFor(l.name)===g.name).sort((a,b)=>g.names.length?g.names.indexOf(a.name)-g.names.indexOf(b.name):0)})).filter(g=>g.name==='Payroll Home'||g.links.length);

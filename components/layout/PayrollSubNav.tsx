...
export default function PayrollSubNav(){
 ...
 const active=location.pathname==='/payroll/home'?'Run Payroll':current?payrollGroupFor(current.name):'Run Payroll';
 ...
 {active==='Schedule Builder'&&detail&&<div className="mt-3 flex flex-wrap gap-2 border-t pt-3 dark:border-slate-700">{detail.links.map(link=><Link key={link.path} to={link.path} className={`min-h-11 rounded-lg border px-4 py-3 text-sm font-semibold ${current?.path===link.path?'border-violet-600 bg-violet-50 text-violet-800 dark:bg-violet-950/40 dark:text-violet-200':'border-slate-200 dark:border-slate-700'}`}>{link.name}</Link>)}</div>}
 {active!=='Payroll Home'&&active!=='Schedule Builder'&&location.pathname!=='/payroll/run'&&detail&&<div className="mt-3 flex flex-wrap items-center gap-3 border-t pt-3 dark:border-slate-700"><label className="text-sm font-medium" htmlFor="payroll-screen">{active}</label><select id="payroll-screen" className="min-h-11 max-w-full rounded-lg border bg-transparent px-3 dark:border-slate-600" value={current?.path||''} onChange={e=>navigate(e.target.value)}>{detail.links.map(l=><option key={l.path} value={l.path}>{l.path==='/payroll/attendance-readiness'?'Timekeeping Review':l.name}</option>)}</select><Link to="/payroll/run" className="text-sm text-violet-600 dark:text-violet-300">Change business unit / cutoff →</Link></div>}
 </div>;
}

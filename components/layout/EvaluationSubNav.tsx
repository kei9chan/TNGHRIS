...
const EvaluationSubNav: React.FC = () => {
    ...
    const canViewEvaluation = can('Evaluation', Permission.View) || hasEvaluationOversightAccess(user);
    const { hasAssignment } = useEvaluationAssignmentAccess(!canViewEvaluation);
    ...
    const subLinks = evaluationLink.children?.filter(
        child => (
            child.requiredPermission && can(child.requiredPermission.resource, child.requiredPermission.permission)
        ) || (child.name === 'Evaluations' && (hasAssignment || canViewEvaluation))
    ) || [];

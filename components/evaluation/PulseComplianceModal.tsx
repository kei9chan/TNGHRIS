...
import {ComplianceRow,ComplianceFilters,emptyComplianceFilters,filterCompliance,complianceExportSheets} from '../../services/pulseCompliance';
...
export default function PulseComplianceModal({surveyId,onClose}:{surveyId:string;onClose:()=>void}){
 ...
 async function download(){setBusy(true);setError('');setNotice('');try{
 const exportFilters={...filters};
 const data=await pulseAudienceRpc('export_pulse_compliance',{p_survey:surveyId,p_employees:filtered.map(r=>r.id),p_filters:exportFilters});
 const {default:ExcelJS}=await import('exceljs');const workbook=new ExcelJS.Workbook();
 for(const [name,rows] of Object.entries(complianceExportSheets(data,exportFilters))){const sheet=workbook.addWorksheet(name);sheet.addRows(rows);sheet.columns.forEach(c=>{c.width=28;});sheet.getRow(1).font={bold:true};}
 const buffer=await workbook.xlsx.writeBuffer();const url=URL.createObjectURL(new Blob([buffer as BlobPart],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}));const a=document.createElement('a');a.href=url;a.download='pulse-survey-compliance.xlsx';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);setNotice('Exported filtered compliance details, summary and survey results.');
 }catch(e){setError((e as Error).message||'Could not export this report. Please try again.');}finally{setBusy(false);}}

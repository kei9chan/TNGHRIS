...
const OfferApprovalPackageModal: React.FC<OfferApprovalPackageModalProps> = ({ isOpen, onClose, offer, candidate, application, ratings, onSubmitted }) => {
  ...
  const loadDocuments = useCallback(async () => {
    ...
    try {
      ...
      setDocuments(next);
      setSelectedIds(current => current.length ? current.filter(id => next.some(document => document.id === id)) : next.filter(document => document.source === 'resume' || document.source === 'offer' || document.source === 'rating').map(document => document.id));
    } catch (reason: any) {
  ...
  const handleUpload = async (file?: File) => {
    ...
    try {
      ...
      setSelectedIds(current => [...current, uploaded.id]);
    } catch (reason: any) {
  ...
  return <Modal isOpen={isOpen} onClose={onClose} title={`Request Offer Approval · ${candidate.firstName} ${candidate.lastName}`} size="5xl">
    <div className="space-y-5">
      ...
      <section className="rounded-xl border border-slate-200 p-4 dark:border-slate-700"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-lg font-bold text-slate-900 dark:text-white">Package checklist</h2><p className="mt-1 text-sm text-slate-500">Select only documents belonging to this candidate.</p></div><div className="flex flex-wrap gap-2 text-sm"><span className={`rounded-full px-3 py-1 font-semibold ${hasResume ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>{hasResume ? '✓' : '○'} Resume attached</span><span className={`rounded-full px-3 py-1 font-semibold ${hasRating ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>{hasRating ? '✓' : '○'} Interview rating(s) attached</span><span className={`rounded-full px-3 py-1 font-semibold ${hasOffer ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>{hasOffer ? '✓' : '○'} Offer attached</span></div></div>
        <div className="mt-4 grid gap-3 md:grid-cols-[1fr_190px]"><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search by document, reviewer, or candidate" className="rounded-lg border border-slate-300 px-3 py-2.5 text-sm dark:border-slate-600 dark:bg-slate-800"/><select value={typeFilter} onChange={event => setTypeFilter(event.target.value)} className="rounded-lg border border-slate-300 px-3 py-2.5 text-sm dark:border-slate-600 dark:bg-slate-800"><option>All</option><option>Resume</option><option>Interview Rating</option><option>Offer</option><option>Other Supporting Document</option></select></div>
        <div className="mt-3 divide-y rounded-xl border border-slate-200 dark:divide-slate-700 dark:border-slate-700">{loading ? <p className="p-5 text-sm text-slate-500">Loading candidate documents…</p> : visibleDocuments.length ? visibleDocuments.map(document => <div key={document.id} className="flex flex-wrap items-center gap-3 p-3"><input type="checkbox" checked={selectedIds.includes(document.id)} disabled={document.isSelectable === false} onChange={() => toggleDocument(document)} aria-label={`Attach ${packageDocumentLabel(document)}`} /><div className="min-w-0 flex-1"><p className="truncate font-semibold text-slate-800 dark:text-slate-100">{packageDocumentLabel(document)}</p><p className="text-xs text-slate-500">{document.documentType}{document.reviewerPosition ? ` · ${document.reviewerPosition}` : ''}{document.status ? ` · ${document.status}` : ''}</p></div><Button size="sm" variant="secondary" onClick={() => void openOfferPackageDocument(document, { candidate, offer, ratings })}>Preview / Download</Button>{document.source === 'candidate_document' && <button type="button" onClick={() => void handleRemove(document)} className="text-sm font-semibold text-rose-600">Remove</button>}</div>) : <p className="p-5 text-sm text-slate-500">No candidate documents match your search.</p>}</div>
        <div className="mt-4 rounded-xl bg-slate-50 p-3 dark:bg-slate-900/60"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-semibold text-slate-800 dark:text-slate-100">Selected attachments</p><p className="text-xs text-slate-500">Reorder the package as it will be reviewed.</p></div><div className="flex flex-wrap items-end gap-2"><select value={documentType} onChange={event => setDocumentType(event.target.value as typeof documentType)} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800"><option>Other Supporting Document</option><option>Resume</option><option>Interview Rating</option><option>Offer</option></select><label className="cursor-pointer rounded-lg bg-white px-3 py-2 text-sm font-semibold text-violet-700 shadow-sm ring-1 ring-inset ring-violet-200 hover:bg-violet-50 dark:bg-slate-800 dark:ring-violet-800">{uploading ? 'Uploading…' : 'Add Existing Candidate Document'}<input type="file" hidden accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" disabled={uploading} onChange={event => { void handleUpload(event.target.files?.[0]); event.currentTarget.value = ''; }}/></label></div></div>{selectedDocuments.length ? <div className="mt-3 space-y-2">{selectedDocuments.map((document, index) => <div key={document.id} className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white p-2 text-sm dark:border-slate-700 dark:bg-slate-800"><span className="w-6 text-center font-bold text-slate-400">{index + 1}</span><span className="min-w-0 flex-1 truncate font-semibold">{packageDocumentLabel(document)}</span><button type="button" onClick={() => moveSelected(index, -1)} disabled={index === 0} className="rounded border px-2 py-1 disabled:opacity-40" aria-label="Move attachment up">↑</button><button type="button" onClick={() => moveSelected(index, 1)} disabled={index === selectedDocuments.length - 1} className="rounded border px-2 py-1 disabled:opacity-40" aria-label="Move attachment down">↓</button><button type="button" onClick={() => toggleDocument(document)} className="rounded border px-2 py-1 text-rose-600" aria-label={`Remove ${document.fileName} from package`}>×</button></div>)}</div> : <p className="mt-3 text-sm text-slate-500">No documents selected yet.</p>}</div>
      </section>
      ...
      <div className="flex flex-wrap justify-end gap-2 border-t border-slate-200 pt-4 dark:border-slate-700"><Button variant="secondary" onClick={onClose}>Close</Button><Button onClick={() => void submit()} isLoading={submitting} disabled={Boolean(success) || loading || uploading}>Request Offer Approval</Button></div>
    </div>
  </Modal>;
};

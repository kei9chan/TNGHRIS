...
import { getSignedAnnouncementUrl } from '../../services/announcementService';
...
const AnnouncementCard: React.FC<AnnouncementCardProps> = ({ announcement, onAcknowledge, hasAcknowledged, canManage, onEdit }) => {
    ...
    useEffect(() => {
        let isMounted = true;
        const load = async () => {
            const url = await getSignedAnnouncementUrl(announcement.attachmentUrl);
                            ...
                            {signedUrl ? (
                                isImage(signedUrl) ? (
                                    ...
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        className="inline-flex items-center text-indigo-600 dark:text-indigo-300 hover:underline break-all text-sm"
                                    >
                                ...
                                <div className="text-xs text-gray-400">Loading attachment…</div>
                            )}
                        </div>
                    )}
                </div>

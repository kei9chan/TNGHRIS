...
import { Ticket, TicketCategory, TicketPriority, TicketStatus, User, Permission, ChatMessage } from '../../types';
...
const TicketModal: React.FC<TicketModalProps> = ({ isOpen, onClose, ticket, onSave, onSendMessage, onResolve, onApproveResolution, onRejectResolution }) => {
  const { user } = useAuth();
  ...
  const handleFile = (file: File) => {
    console.log("Attachment uploaded for ticket:", file.name);
    setCurrentTicket(prev => ({ ...prev, attachments: [file.name] }));
  };
  ...
  const handleSave = () => {
    ...
    const assignedUser = mockUsers.find(u => u.id === currentTicket.assignedToId);
  ...
  const filteredAssignees = useMemo(() => {
    ...
    if (currentTicket.assignedToName && lowerSearch === currentTicket.assignedToName.toLowerCase()) {
        return mockUsers.filter(u => u.name.toLowerCase().includes(lowerSearch) && u.id !== currentTicket.assignedToId);
    }
    
    return mockUsers.filter(u => u.name.toLowerCase().includes(lowerSearch));
  }, [assigneeSearch, currentTicket.assignedToId, currentTicket.assignedToName]);
  ...
  const renderFooter = () => {
      const isAssignee = canManage || user?.id === currentTicket.assignedToId;
      ...
      if (isAssignee && (currentTicket.status === TicketStatus.Assigned || currentTicket.status === TicketStatus.InProgress)) {
        return (
            <div className="flex justify-end w-full space-x-2">
                <Button variant="secondary" onClick={onClose}>Cancel</Button>
      ...
      if (canManage && currentTicket.status !== TicketStatus.Resolved && currentTicket.status !== TicketStatus.Closed) {
           return (
            <div className="flex justify-end w-full space-x-2">
                <Button variant="secondary" onClick={onClose}>Cancel</Button>
                <Button onClick={handleSave}>Save Changes</Button>
  ...
  const canChat = currentTicket.id && user && (user.id === currentTicket.requesterId || user.id === currentTicket.assignedToId || canManage);
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <select id="priority" name="priority" value={currentTicket.priority || ''} onChange={handleChange} disabled={!canManage && !isNewTicket} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white disabled:bg-gray-200 dark:disabled:bg-gray-800">
                ...
                <Textarea label="Description" id="description" name="description" value={currentTicket.description || ''} onChange={handleChange} rows={5} required disabled={!isNewTicket} />

...
export default function ApprovalWidget() {
  ...
  return <section aria-labelledby="approval-inbox-title" className="mb-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800 sm:p-6">
    ...
    <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{groups.map(({kind,requests}) => <Link key={kind} to={`/approvals?type=${kind}`} aria-label={`Review ${labels[kind]}: ${requests.length} pending`} className="flex min-h-24 items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 hover:border-violet-500 focus-visible:outline focus-visible:outline-2 focus-visible:outline-violet-500 dark:border-slate-600 dark:bg-slate-900/40">
      <div><h3 className="font-bold text-slate-900 dark:text-white">{labels[kind]}</h3><p className="mt-1 text-sm text-slate-500 dark:text-slate-300">{requests.length} pending approval{requests.length === 1 ? '' : 's'}</p></div><span className="inline-flex min-w-10 items-center justify-center rounded-full bg-violet-100 px-3 py-2 font-bold text-violet-900 dark:bg-violet-900 dark:text-violet-100">{requests.length}</span>
    </Link>)}</div>

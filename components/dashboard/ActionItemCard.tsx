...
const ActionItemCard: React.FC<ActionItemCardProps> = ({ icon, title, subtitle, date, link, colorClass, onClick }) => {
    const content = (
    ...
    if (onClick) {
        return (
            <button type="button" onClick={onClick} className="block w-full text-left hover:no-underline">
                {content}
            </button>
        );
    }

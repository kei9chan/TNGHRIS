// Phase A complete: mockDataCompat removed from RecentMemosWidget
...
const RecentMemosWidget: React.FC<RecentMemosWidgetProps> = ({ onViewMemo }) => {
    ...
    const { recentMemos, actionRequiredCount } = useMemo(() => {
        ...
        const count = publishedMemos.filter(memo =>
            memo.acknowledgementRequired && !memo.acknowledgementTracker.includes(user.id)
        ).length;
    ...
    return (
        <Card title="Recent Memos & Announcements">
            <div className="space-y-4">
                {isLoading ? (
                    <p className="text-sm text-gray-500 dark:text-gray-400">Loading memos…</p>
                ) : (
                    <>

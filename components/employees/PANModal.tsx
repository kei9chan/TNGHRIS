...
const PANModal: React.FC<PANModalProps> = (props) => {
  ...
  const templateOptions = useMemo(() => {
    ...
    return templates.filter(item => !item.businessUnitId || !buId || item.businessUnitId === buId).sort((a, b) => a.name.localeCompare(b.name));
  ...
  useEffect(() => {
    ...
    if (pan) {
      ...
      setSelectedTemplateId(pan.templateId || ''); setEmployeeSearch(pan.employeeName || '');
      ...
      setSelectedEmployee(null); setSelectedTemplateId(''); setSelectedApprovers([]); setEmployeeSearch('');
    }
  ...
  const selectEmployee = (employee: User) => { setSelectedEmployee(employee); setCurrent(previous => ({ ...previous, employeeId: employee.id, employeeName: employee.name })); setEmployeeSearch(employee.name); setIsEmployeeSearchOpen(false); };
  ...
  return <Modal isOpen={isOpen} onClose={onClose} title={pan ? `${canEdit ? 'Edit' : 'PAN Details'} — ${pan.employeeName}` : 'Create New PAN'} size="lg" footer={
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex flex-wrap gap-2"><Button variant="secondary" onClick={onClose}>Close</Button>{pan && <Button variant="secondary" onClick={() => onDownloadPdf(pan)}>Print / Download</Button>}{canCancel && onCancel && <Button variant="danger" onClick={() => onCancel(pan!)}>Cancel PAN</Button>}</div>
    ...
    <div className="space-y-6">
      {pan && <div className="flex flex-wrap items-start justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800"><div><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">PAN reference</p><p className="mt-1 text-lg font-bold text-slate-900 dark:text-white">PAN-{pan.id.slice(0, 8).toUpperCase()}</p></div><span className={`rounded-full px-3 py-1 text-sm font-semibold ${statusClass}`}>{statusLabel}</span></div>}
      ...
      <section><div className="mb-3 flex items-center gap-2"><b className="flex h-7 w-7 items-center justify-center rounded-full bg-indigo-600 text-xs text-white">3</b><div><h3 className="font-semibold">Employment details</h3><p className="text-xs text-slate-500">These From / To fields are used by the printable form.</p></div></div><div className="grid gap-4 lg:grid-cols-2">{(['from', 'to'] as const).map(side => <div key={side} className="rounded-xl border bg-white p-4 dark:bg-slate-900"><div className="mb-4 flex items-center justify-between"><h4 className="font-semibold">{side === 'from' ? 'From / Current' : 'To / New'}</h4><span className="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-500">{side === 'from' ? 'Existing' : 'Proposed'}</span></div><div className="space-y-3">{valueField(side, 'Business Unit / Company', 'businessUnit')}{valueField(side, 'Department', 'department')}{valueField(side, 'Position', 'position')}{valueField(side, 'Employment status', 'employmentStatus')}</div></div>)}</div></section>

      <section><div className="mb-3 flex items-center gap-2"><b className="flex h-7 w-7 items-center justify-center rounded-full bg-indigo-600 text-xs text-white">4</b><div><h3 className="font-semibold">Compensation</h3><p className="text-xs text-slate-500">Amounts are shown in the printed From / To table.</p></div></div><div className="grid gap-4 lg:grid-cols-2">{(['from', 'to'] as const).map(side => <div key={side} className="rounded-xl border bg-white p-4 dark:bg-slate-900"><h4 className="mb-3 font-semibold">{side === 'from' ? 'From / Current' : 'To / New'}</h4><div className="grid gap-3 sm:grid-cols-3">{(['basic', 'deminimis', 'reimbursable'] as const).map(field => <Input key={field} label={field === 'deminimis' ? 'De minimis' : field[0].toUpperCase() + field.slice(1)} type="number" value={current.particulars?.[side]?.salary?.[field] ?? 0} onChange={event => updateSalary(side, field, event.target.value)} disabled={!canEdit} />)}</div></div>)}</div></section>

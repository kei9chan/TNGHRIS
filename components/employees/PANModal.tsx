...
import { PAN, User, SalaryBreakdown, SalaryPayBasis, PANActionTaken, PANStatus, PANTemplate, Role, PANRoutingStep, PANStepStatus, PANRole } from '../../types';
...
interface PANModalProps {
  ...
  onSaveDraft: (pan: Partial<PAN>) => Promise<PAN>; onSendForAcknowledgement: (pan: Partial<PAN>) => Promise<PAN>;
...
const emptySalary: SalaryBreakdown = { basic: 0, deminimis: 0, reimbursable: 0, payBasis: 'gross' };
...
const PANModal: React.FC<PANModalProps> = (props) => {
  ...
  const save = async (submit: boolean) => {
    if (saveLock.current || sent) return;
    const errors: Record<string,string> = {};
    if (!current.employeeId) errors.employee = 'Select an employee from the directory.';
    if (!current.effectiveDate || Number.isNaN(new Date(current.effectiveDate).getTime())) errors.date = 'Enter a valid effectivity date.';
    if (submit && getPANActionType(current.actionTaken) === 'general') errors.action = 'Select at least one personnel action.';
    if (submit && !current.routingSteps?.length) errors.routing = 'Select the required approvers.';
    setFieldErrors(errors);
    if (Object.keys(errors).length) { setSaveError('Check the highlighted fields. Your entries have been kept.'); return; }
    saveLock.current = true; setSaving(true); setSaveError(''); setSaveMessage('');
    const candidate = { ...current, id: current.id || crypto.randomUUID() };
    setCurrent(candidate);
    try {
      const saved = await (submit ? onSendForAcknowledgement(candidate) : onSaveDraft(candidate));
      setCurrent(previous => ({ ...previous, ...saved }));
      setSent(submit);
      setSaveMessage(submit ? `PAN sent for approval. Status: ${saved.status}.` : 'PAN draft saved. You can now send it for approval.');
    } catch (error: any) {
      if (error.savedPan) setCurrent(previous => ({ ...previous, id: error.savedPan.id, createdByUserId: error.savedPan.createdByUserId, status: PANStatus.Draft }));
      setSaveError(error?.message || 'PAN could not be saved. Your entries have been kept.');
  ...
  useEffect(() => {
    if (!isOpen) return;
    const key = `${user?.id}:${pan?.id || 'new'}:${pan?.status || 'draft'}`;
    if (initialized.current === key && !(isNew && sent)) return;
    initialized.current = key;
    setSaveError(''); setSaveMessage(''); setSent(false);
    if (pan) {
      ...
      setSelectedApprovers(pan.routingSteps.map(step => approvers.find(item => item.id === step.userId) || employees.find(item => item.id === step.userId)).filter((item): item is User => !!item));
  ...
  }, [pan?.id, pan?.status, user?.id, isOpen]);
  ...
  const updateSalary = (side: 'from' | 'to', field: keyof SalaryBreakdown, value: string) => updateParticular(side, 'salary', { ...(current.particulars?.[side]?.salary || emptySalary), [field]: value as unknown as number });
  const updateSalaryBasis = (side: 'from' | 'to', payBasis: SalaryPayBasis) => updateParticular(side, 'salary', { ...(current.particulars?.[side]?.salary || emptySalary), payBasis });
  ...
  const fromSalary = current.particulars?.from?.salary || emptySalary;
  const toSalary = current.particulars?.to?.salary || emptySalary;
  const salaryTotal = (value: SalaryBreakdown) => Number(value.basic || 0) + Number(value.deminimis || 0) + Number(value.reimbursable || 0);
  const compensationDifference = salaryTotal(toSalary) - salaryTotal(fromSalary);
  const panCompensationApproved = !!pan?.approvalCompletedAt && [PANStatus.PendingEmployee, PANStatus.Completed].includes(pan.status);
  const cutoffLabel = current.effectiveDate ? (() => { const date = new Date(current.effectiveDate); const day = date.getDate(); return day >= 11 && day <= 25 ? '11–25 cutoff → payroll release on the 5th' : '26–10 cutoff → payroll release on the 20th'; })() : 'Set an effective date';

  const valueField = (side: 'from' | 'to', label: string, field: 'businessUnit' | 'department' | 'position' | 'employmentStatus') => (
    <div className="space-y-1">
      <label className="text-xs font-semibold uppercase tracking-wide text-slate-500">{label}</label>
      {field === 'businessUnit' ? <select value={current.particulars?.[side]?.businessUnitId || ''} onChange={event => handleBusinessUnitChange(side, event.target.value)} disabled={!canEdit || saving || sent} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm dark:border-slate-600 dark:bg-slate-900 dark:text-white"><option value="">{current.particulars?.[side]?.businessUnit || 'Not Applicable'}</option>{businessUnits.map(unit => <option key={unit.id} value={unit.id}>{unit.name}</option>)}</select> : <Input value={current.particulars?.[side]?.[field] || ''} onChange={event => updateParticular(side, field, event.target.value)} disabled={!canEdit || saving || sent} />}
    </div>
  );

  return <Modal isOpen={isOpen} onClose={saving ? () => {} : onClose} title={pan ? `${canEdit ? 'Edit' : 'PAN Details'} — ${pan.employeeName}` : 'Create New PAN'} size="5xl" viewportFit footer={
    <div>
      {saveError && <p role="alert" className="mb-3 rounded-lg bg-red-50 p-3 text-red-800 dark:bg-red-950 dark:text-red-200">{saveError}</p>}
      {saveMessage && <p role="status" className="mb-3 rounded-lg bg-emerald-50 p-3 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200">{saveMessage} Saved ID: {current.id}</p>}
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex flex-wrap gap-2"><Button variant="secondary" disabled={saving} onClick={onClose}>Close</Button>{pan && <Button variant="secondary" onClick={() => onDownloadPdf(pan)}>Preview / Print / PDF</Button>}{canCancel && onCancel && <Button variant="danger" disabled={saving} onClick={() => onCancel(pan!)}>Cancel PAN</Button>}</div>
      <div className="flex flex-wrap gap-2 sm:justify-end">{canEdit && <Button variant="secondary" disabled={saving || sent} isLoading={saving} onClick={() => void save(false)}>Save Draft</Button>}{canEdit && <Button disabled={saving || sent} isLoading={saving} onClick={() => void save(true)}>Send for Approval</Button>}{currentUserStep && <><Button variant="secondary" onClick={() => { onReject?.(pan!); onClose(); }}>Reject</Button><Button onClick={() => { onApprove?.(pan!); onClose(); }}>Approve</Button></>}{isForAcknowledgement && <Button onClick={signatureSubmit}>Acknowledge</Button>}</div>
    </div></div>
  }>
    <fieldset disabled={saving || sent} className="min-w-0 space-y-6">
      ...
      <section><div className="mb-3 flex items-center gap-2"><b className="flex h-7 w-7 items-center justify-center rounded-full bg-indigo-600 text-xs text-white">1</b><div><h3 className="font-semibold">Document setup</h3><p className="text-xs text-slate-500">Choose the employee, effectivity date, and business-unit template.</p></div></div><div className="grid gap-4 rounded-xl border border-slate-200 bg-white p-4 sm:grid-cols-3 dark:border-slate-700 dark:bg-slate-900">
        <div className="relative sm:col-span-2" ref={searchWrapperRef}><label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500">Employee</label><Input aria-label="Employee" error={fieldErrors.employee} aria-invalid={!!fieldErrors.employee} value={employeeSearch} onChange={event => handleEmployeeSearchChange(event.target.value)} onFocus={() => setIsEmployeeSearchOpen(true)} placeholder="Search name, employee ID, email, position, department, or business unit" disabled={!canEdit || saving || sent} />{directoryError && <p role="alert" className="mt-2 text-sm font-semibold text-red-600">Employee directory unavailable: {directoryError}</p>}{selectedEmployee && <div className="mt-2 flex items-center justify-between rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm dark:border-emerald-800 dark:bg-emerald-950/30"><div><span className="font-semibold text-emerald-800 dark:text-emerald-200">Selected: {selectedEmployee.name}</span><span className="ml-2 text-xs text-emerald-700 dark:text-emerald-300">{[selectedEmployee.employeeId, selectedEmployee.position, selectedEmployee.department, selectedEmployee.businessUnit].filter(Boolean).join(' · ')}</span></div>{canEdit && <button type="button" onClick={clearEmployee} className="font-semibold text-emerald-800 hover:underline dark:text-emerald-200">Clear</button>}</div>}{isEmployeeSearchOpen && employeeSearch.trim() && employeeSearch !== selectedEmployee?.name && <div className="absolute left-0 right-0 top-full z-20 mt-1 max-h-72 overflow-y-auto rounded-lg border bg-white shadow-xl dark:border-slate-700 dark:bg-slate-800">{availableEmployees.length > 0 ? availableEmployees.map(employee => <button type="button" key={employee.id} onClick={() => selectEmployee(employee)} className="block w-full border-b border-slate-100 px-3 py-2 text-left text-sm last:border-0 hover:bg-indigo-50 dark:border-slate-700 dark:hover:bg-slate-700"><span className="block font-medium">{employee.name}</span><span className="block text-xs text-slate-500 dark:text-slate-400">{[employee.employeeId, employee.position || employee.role, employee.department, employee.businessUnit, employee.email].filter(Boolean).join(' · ')}</span></button>) : <div className="px-3 py-3 text-sm text-slate-500">{directoryError ? 'The employee directory failed to load. Refresh after the configuration error is resolved.' : 'No active employee matches this search in your authorized business-unit scope.'}</div>}</div>}</div>
        <Input error={fieldErrors.date} aria-invalid={!!fieldErrors.date} label="Effectivity date" type="date" value={current.effectiveDate && !Number.isNaN(new Date(current.effectiveDate).getTime()) ? new Date(current.effectiveDate).toISOString().split('T')[0] : ''} onChange={event => setCurrent(previous => ({ ...previous, effectiveDate: event.target.value ? new Date(event.target.value) : undefined }))} disabled={!canEdit || saving || sent} />
        <div className="sm:col-span-3"><label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500">PAN template</label><select value={selectedTemplateId} onChange={handleTemplateSelect} disabled={!canEdit || saving || sent} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-white"><option value="">Use standard PAN layout</option>{templateOptions.map(template => <option key={template.id} value={template.id}>{template.name}{template.businessUnitName ? ` — ${template.businessUnitName}` : ' — Global'}</option>)}</select><p className="mt-1 text-xs text-slate-500">{selectedEmployee?.businessUnit ? `Showing ${selectedEmployee.businessUnit} and Global templates.` : 'Select an employee to prioritize its business-unit templates.'}</p></div>
      </div></section>

      <section><div className="mb-3 flex items-center gap-2"><b className="flex h-7 w-7 items-center justify-center rounded-full bg-indigo-600 text-xs text-white">2</b><div><h3 className="font-semibold">Action taken</h3>{fieldErrors.action && <p role="alert" className="text-sm text-red-600 dark:text-red-300">{fieldErrors.action}</p>}<p className="text-xs text-slate-500">Select every change that applies.</p></div></div><div className="grid gap-2 rounded-xl border bg-slate-50 p-4 sm:grid-cols-2 dark:bg-slate-800">{actionLabels.map(([key, label]) => <label key={key} className="flex items-center gap-3 rounded-lg border bg-white px-3 py-2 text-sm dark:bg-slate-900"><input type="checkbox" checked={!!current.actionTaken?.[key]} disabled={!canEdit || saving || sent} onChange={event => setCurrent(previous => ({ ...previous, actionTaken: { ...emptyActions, ...previous.actionTaken, [key]: event.target.checked } }))} className="h-4 w-4 text-indigo-600" /><span>{label}</span></label>)}<div className="flex items-center gap-2 rounded-lg border bg-white px-3 py-2 dark:bg-slate-900"><input type="checkbox" checked={!!current.actionTaken?.others} disabled={!canEdit || saving || sent} onChange={event => setCurrent(previous => ({ ...previous, actionTaken: { ...emptyActions, ...previous.actionTaken, others: event.target.checked ? previous.actionTaken?.others || 'Other action' : '' } }))} className="h-4 w-4 text-indigo-600" /><Input value={current.actionTaken?.others || ''} onChange={event => setCurrent(previous => ({ ...previous, actionTaken: { ...emptyActions, ...previous.actionTaken, others: event.target.value } }))} placeholder="Other action" disabled={!canEdit || saving || sent} /></div></div></section>

      <section><div className="mb-3 flex items-center gap-2"><b className="flex h-7 w-7 items-center justify-center rounded-full bg-indigo-600 text-xs text-white">3</b><div><h3 className="font-semibold">From vs To details</h3><p className="text-xs text-slate-500">The complete transfer and employment comparison used by the printable form.</p></div></div><div className="grid gap-4 lg:grid-cols-2">{(['from', 'to'] as const).map(side => <div key={side} className="rounded-xl border bg-white p-4 dark:bg-slate-900"><div className="mb-4 flex items-center justify-between"><h4 className="font-semibold">{side === 'from' ? 'From / Current' : 'To / New'}</h4><span className="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-500">{side === 'from' ? 'Existing' : 'Proposed'}</span></div><div className="space-y-3">{valueField(side, 'Business Unit / Company', 'businessUnit')}{valueField(side, 'Department', 'department')}{valueField(side, 'Position', 'position')}{valueField(side, 'Employment status', 'employmentStatus')}<Input label="Other business units / affiliates" value={otherBusinessUnitText[side]} onChange={event => handleOtherBusinessUnitsChange(side, event.target.value)} placeholder="Separate multiple companies with commas" disabled={!canEdit || saving || sent} /></div></div>)}</div></section>

      {showSalary && <section><div className="mb-3 flex items-center gap-2"><b className="flex h-7 w-7 items-center justify-center rounded-full bg-indigo-600 text-xs text-white">4</b><div><h3 className="font-semibold">Salary package</h3><p className="text-xs text-slate-500">Choose who shoulders withholding tax. Amounts are shown in PHP in the printed From / To table.</p></div></div><div className="grid gap-4 lg:grid-cols-2">{(['from', 'to'] as const).map(side => { const payBasis = current.particulars?.[side]?.salary?.payBasis || 'gross'; return <div key={side} className="rounded-xl border bg-white p-4 dark:bg-slate-900"><h4 className="mb-3 font-semibold">{side === 'from' ? 'From / Current' : 'To / New'}</h4><fieldset className="mb-4"><legend className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Salary basis</legend><div className="grid gap-2 sm:grid-cols-2"><label className={`rounded-lg border p-3 text-sm ${payBasis === 'gross' ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-950/40' : 'border-slate-200 dark:border-slate-700'}`}><input className="mr-2" type="radio" name={`${side}-salary-basis`} value="gross" checked={payBasis === 'gross'} onChange={() => updateSalaryBasis(side, 'gross')} disabled={!canEdit || saving || sent} /><strong>Gross</strong><span className="mt-1 block text-xs text-slate-500">Employee shoulders withholding tax.</span></label><label className={`rounded-lg border p-3 text-sm ${payBasis === 'net_tax' ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-950/40' : 'border-slate-200 dark:border-slate-700'}`}><input className="mr-2" type="radio" name={`${side}-salary-basis`} value="net_tax" checked={payBasis === 'net_tax'} onChange={() => updateSalaryBasis(side, 'net_tax')} disabled={!canEdit || saving || sent} /><strong>Net</strong><span className="mt-1 block text-xs text-slate-500">Employer shoulders withholding tax.</span></label></div></fieldset><div className="grid gap-3 sm:grid-cols-3">{(['basic', 'deminimis', 'reimbursable'] as const).map(field => <Input key={field} label={field === 'deminimis' ? 'De minimis' : field[0].toUpperCase() + field.slice(1)} type="text" inputMode="decimal" unit="PHP" value={current.particulars?.[side]?.salary?.[field] ?? 0} onChange={event => updateSalary(side, field, event.target.value)} disabled={!canEdit || saving || sent} />)}</div></div>; })}</div></section>}

      {showSalary && <section className={`rounded-xl border p-5 ${panCompensationApproved ? 'border-emerald-200 bg-emerald-50' : 'border-violet-200 bg-violet-50'}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg font-bold">Compensation impact review</h3><p className="text-sm text-slate-600">One approval updates payroll and the employee profile; there is no second package approval.</p></div>{pan && <Button size="sm" variant="secondary" onClick={() => onDownloadPdf(pan)}>View supporting PAN document</Button>}</div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-lg bg-white p-3"><span className="text-xs text-slate-500">Current compensation</span><strong className="mt-1 block">PHP {salaryTotal(fromSalary).toLocaleString()}</strong></div><div className="rounded-lg bg-white p-3"><span className="text-xs text-slate-500">Proposed compensation</span><strong className="mt-1 block">PHP {salaryTotal(toSalary).toLocaleString()}</strong></div><div className="rounded-lg bg-white p-3"><span className="text-xs text-slate-500">Difference</span><strong className={`mt-1 block ${compensationDifference >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>{compensationDifference >= 0 ? '+' : ''}PHP {compensationDifference.toLocaleString()}</strong></div><div className="rounded-lg bg-white p-3"><span className="text-xs text-slate-500">Effective date</span><strong className="mt-1 block">{current.effectiveDate ? new Date(current.effectiveDate).toLocaleDateString() : 'Missing'}</strong></div></div><div className="mt-3 grid gap-3 text-sm md:grid-cols-2"><p><strong>Tax impact:</strong> {toSalary.payBasis === 'net_tax' ? 'Employer shoulders withholding tax' : 'Employee shoulders withholding tax'}</p><p><strong>Affected payroll cutoff:</strong> {cutoffLabel}</p><p><strong>Employee-paid benefits:</strong> Preserved from the active package unless changed by this PAN</p><p><strong>Employer-paid benefits / company cost:</strong> Recalculated by the approved payroll engine from the effective date</p></div>{panCompensationApproved && <div className="mt-4 rounded-lg bg-white p-4 text-sm text-emerald-900"><strong>PAN approved</strong><ul className="mt-2 grid gap-1 sm:grid-cols-2"><li>✓ Pay package updated automatically</li><li>✓ Employee compensation profile updated</li><li>✓ Available to payroll from the effective date</li><li>✓ Previous package preserved in salary history</li></ul></div>}</section>}

      <section className="grid gap-4 lg:grid-cols-2"><div><label className="mb-1 block text-sm font-semibold">Remarks / Justifications</label><Textarea value={current.notes || ''} onChange={event => setCurrent(previous => ({ ...previous, notes: event.target.value }))} rows={5} placeholder="Explain the reason for this personnel action." disabled={!canEdit || saving || sent} /></div><div><label className="mb-1 block text-sm font-semibold">Approval routing</label>{fieldErrors.routing && <p role="alert" className="text-sm text-red-600 dark:text-red-300">{fieldErrors.routing}</p>}{canEdit ? <><EmployeeMultiSelect label="" allUsers={approverPool} selectedUsers={selectedApprovers} onSelectionChange={handleApproverSelect} disabled={saving || sent} /><p className={`mt-2 text-xs ${hasBodApprover ? 'text-emerald-600' : 'font-semibold text-amber-600'}`}>{hasBodApprover ? '✓ Board of Director approval included' : 'Add at least one Board of Director approver.'}</p></> : <ol className="space-y-2 rounded-lg border border-slate-300 p-3 text-sm dark:border-slate-600">{[...(pan?.routingSteps || [])].sort((a,b) => a.order-b.order).map(step => <li key={step.id} className="flex flex-wrap justify-between gap-2"><span>{step.name}</span><span className="text-slate-500 dark:text-slate-300">{step.role} · {step.status}</span></li>)}</ol>}</div></section>

      {pan && <section className="rounded-xl border bg-slate-50 p-4 dark:bg-slate-800"><h3 className="mb-3 font-semibold">Approval history</h3><div className="grid gap-2 sm:grid-cols-2">{[...pan.routingSteps].sort((a, b) => a.order - b.order).map(step => <div key={step.id} className="rounded-lg border bg-white p-3 text-sm dark:bg-slate-900"><div className="font-medium">{step.order + 1}. {step.name}</div><div className="text-xs text-slate-500">{step.role} · {step.status}{step.timestamp ? ` · ${new Date(step.timestamp).toLocaleString()}` : ''}</div>{step.notes && <div className="mt-1 text-xs text-slate-600 dark:text-slate-300">{step.notes}</div>}</div>)}</div>{rejectionInfo && <p className="mt-3 rounded-lg bg-red-50 p-3 text-sm text-red-700"><strong>Rejected by {rejectionInfo.name}:</strong> {pan.rejectionReason || rejectionInfo.notes || 'No reason recorded.'}</p>}{pan.status === PANStatus.Cancelled && <p className="mt-3 rounded-lg bg-red-50 p-3 text-sm text-red-700"><strong>Cancelled{cancelledByName ? ` by ${cancelledByName}` : ''}:</strong> {pan.cancellationReason || 'No reason recorded.'}</p>}</section>}

import {DayTag,statusPresets} from '../../services/scheduleStatuses';
...
interface ShiftAssignmentDrawerProps {
    onStatus:(tag:DayTag|null)=>void;
    busy?:boolean;error?:string;onRetry?:()=>void;
    isOpen: boolean;
    onClose: () => void;
    employee: User | null;
    date: Date | null;
...
const ShiftAssignmentDrawer: React.FC<ShiftAssignmentDrawerProps> = ({ busy=false,error,onRetry,onStatus,isOpen, onClose, employee, date, templates, onSave, onCopyLastWeekSchedule, onCopyRestOfWeek, canCopyRestOfWeek, gaps }) => {
    const {user}=useAuth();
        ...
        <>
            ...
            >
                <div className="flex flex-col h-full">
                    {busy&&<p role="status" className="p-3">Saving schedule…</p>}
                    {error&&<div role="alert" className="p-3 text-red-500">{error}<button disabled={busy} onClick={onRetry} className="ml-2 underline">Retry</button></div>}
                    <div className="flex justify-between items-center p-4 border-b border-gray-200 dark:border-slate-700 shrink-0">
                        ...
                        <button onClick={()=>{if(!busy)onClose();}} className="p-1 rounded-full text-gray-400 hover:bg-gray-200 dark:hover:bg-slate-700">
                            <XIcon />
                        </button>
                    </div>
                    ...
                    <div className="flex-1 min-h-0 overflow-y-auto p-2">
                        ...
                        <div>
                            <button
                                disabled={busy||!!error} onClick={handleCopyClick}
                            ...
                            <button onClick={onCopyRestOfWeek} disabled={busy||!!error||!canCopyRestOfWeek} className="mt-2 min-h-11 w-full rounded-md border border-violet-400 px-3 py-2 text-sm font-semibold text-violet-700 dark:text-violet-200 disabled:opacity-50">Copy this schedule to the rest of the week</button>
                        ...
                        <div className="mb-3 space-y-2"><h4 className="font-semibold">Status Presets</h4>{statusPresets.filter(p=>p.tag!=='suspended'||canSuspend).map(p=><button key={p.tag} className={`min-h-11 w-full rounded p-2 text-left ${p.color}`} disabled={busy||!!error} onClick={()=>onStatus(p.tag)}>{p.label}</button>)}</div><ul className="space-y-1 pb-4">
                            ...
                            {templates.map(template => {
                                ...
                                return (
                                    <li key={template.id}>
                                        <button
                                            disabled={busy} onClick={() => onSave(employee.id, date, template.id)}
                                        ...
                                        >
                                            <div className="flex justify-between items-center">
                                                <p className="font-medium">{template.name} · {scheduleLabel(template)}</p>

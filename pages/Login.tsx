  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err instanceof DeviceConflictError) {
        ...
        setError(
          'You are already signed in on another device. Force sign in here to log out your other session.'
        );
  ...
  const handleForceSubmit = async () => {
    ...
    try {
      ...
      if (err instanceof SupabaseAuthError) {
        ...
        console.error('[Login] forceLogin error', err);
    ...
    <div className="min-h-screen flex overflow-hidden bg-gray-50 font-sans">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 relative">
        ...
        <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl p-8 md:p-12 relative z-10 border border-gray-100">
          ...
          {successMessage && (
            <div
              className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-xl relative mb-6 flex items-center animate-pulse"
              role="alert"
            >
              <svg
                ...
                <div className="relative">
                  ...
                  <div
                    className={`block w-10 h-6 rounded-full transition-colors ${
                      rememberMe ? 'bg-violet-500' : 'bg-gray-300'
                    }`}
                  ></div>
                  <div

// Phase A complete: mockDataCompat removed from EmployeeAnalytics
...
const EmployeeAnalytics: React.FC = () => {
    const [users, setUsers] = useState<{ status: string; department: string; dateHired?: string }[]>([]);
        ...
        const load = async () => {
            try {
                const { data } = await supabase
                    .from('hris_users')
                    .select('status, department, date_hired');
                if (data) setUsers(data.map((u: any) => ({ status: u.status, department: u.department, dateHired: u.date_hired })));
            } catch (err) {
                console.error('Failed to load users for analytics', err);
            }
        };
        load();
    }, []);
    ...
    const turnoverRate = useMemo(() => {
        const leavers = users.filter(u => u.status === 'Inactive').length;
        const total = users.length;
        return total > 0 ? (leavers / total) * 100 : 0;
    }, [users]);

    const tenureDistribution = useMemo(() => {
        const distribution = { '< 1 Year': 0, '1-3 Years': 0, '3-5 Years': 0, '5+ Years': 0 };
        const now = new Date();
        users.filter(u => u.status === 'Active' && u.dateHired).forEach(user => {

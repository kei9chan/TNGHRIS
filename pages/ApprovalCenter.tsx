...
export default function ApprovalCenter() {
  ...
  return <div className="space-y-5 pb-12">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[['Pending approvals', filtered.length, 'bg-blue-50 text-blue-700'], ['Due today', dueTodayCount, 'bg-orange-50 text-orange-700'], ['Overdue', overdueCount, 'bg-red-50 text-red-700'], ['Exceptions', exceptionCount, 'bg-amber-50 text-amber-700']].map(([label, value, color]) => <div key={String(label)} className="rounded-xl border bg-white p-5 shadow-sm dark:bg-slate-800"><div className={`inline-flex rounded-lg px-3 py-1 text-2xl font-bold ${color}`}>{value}</div><p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{label}</p></div>)}</div>
    <div className="rounded-xl border bg-white p-4 shadow-sm dark:bg-slate-800">
      <div className="mb-3 grid gap-3 sm:grid-cols-3"><label className="text-sm font-semibold text-slate-700">Date from<input type="date" aria-label="Date from" value={filters.dateFrom} onChange={event => setFilters({ ...filters, dateFrom: event.target.value })} className="mt-1 block w-full rounded-lg border px-3 py-2 font-normal" /></label><label className="text-sm font-semibold text-slate-700">Date to<input type="date" aria-label="Date to" value={filters.dateTo} onChange={event => setFilters({ ...filters, dateTo: event.target.value })} className="mt-1 block w-full rounded-lg border px-3 py-2 font-normal" /></label><label className="text-sm font-semibold text-slate-700">Approver scope<select aria-label="Approver scope" disabled className="mt-1 block w-full rounded-lg border bg-slate-50 px-3 py-2 font-normal"><option>My authorized scope</option></select></label></div>
      <div className="grid gap-3 md:grid-cols-4 xl:grid-cols-8"><input aria-label="Approval search" value={filters.search} onChange={event => setFilters({ ...filters, search: event.target.value })} placeholder="Employee, NTE, PAN, case or request ID" className="rounded-lg border px-3 py-2 md:col-span-2" /><select aria-label="Business unit" value={filters.businessUnit} onChange={event => setFilters({ ...filters, businessUnit: event.target.value })} className="rounded-lg border px-3 py-2"><option value="">All business units</option>{businessUnits.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select><select aria-label="Department" value={filters.department} onChange={event => setFilters({ ...filters, department: event.target.value })} className="rounded-lg border px-3 py-2"><option value="">All departments</option>{departments.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select><select aria-label="Request type" value={filters.kind} onChange={event => setFilters({ ...filters, kind: event.target.value })} className="rounded-lg border px-3 py-2"><option value="">All request types</option>{GROUP_ORDER.map(id => <option key={id} value={id}>{KIND_META[id].title}</option>)}</select><select aria-label="Approval status" value={filters.status} onChange={event => setFilters({ ...filters, status: event.target.value })} className="rounded-lg border px-3 py-2"><option value="">All statuses</option>{statuses.map(status => <option key={status} value={status}>{status}</option>)}</select><select aria-label="Age of request" value={filters.age} onChange={event => setFilters({ ...filters, age: event.target.value })} className="rounded-lg border px-3 py-2"><option value="">Any age</option><option value="today">Due today</option><option value="overdue">Overdue (3+ days)</option></select><select aria-label="Sort approvals" value={filters.sort} onChange={event => setFilters({ ...filters, sort: event.target.value })} className="rounded-lg border px-3 py-2"><option value="newest">Newest first</option><option value="oldest">Oldest first</option><option value="age">Days pending</option></select></div>

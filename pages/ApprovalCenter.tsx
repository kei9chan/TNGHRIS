...
const groupDisplayTitle = (kind: Kind, groupItems: ApprovalItem[]) => {
  ...
  const bodCount = groupItems.filter(item => item.route === 'BOD_REQUIRED').length;
  if (bodCount === groupItems.length) return `${KIND_META[kind].title} — BOD approval required`;
  if (bodCount > 0) return `${KIND_META[kind].title} approvals`;
  return `Standard ${KIND_META[kind].title}`;
};
...
const ApprovalMobileCard: React.FC<{
  ...
  return <article className={`rounded-xl border p-4 shadow-sm ${requested ? 'border-indigo-400 bg-indigo-50 ring-1 ring-indigo-300 dark:bg-indigo-950' : 'border-slate-200 bg-white dark:border-slate-600 dark:bg-slate-800'}`}>
    <div className="flex items-start justify-between gap-3">
      ...
      {item.bulkEligible && <label className="flex min-h-11 flex-shrink-0 items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-200"><input type="checkbox" aria-label={`Select ${item.reference} for bulk approval`} checked={selected} onChange={event => onSelect(event.target.checked)} /> Select</label>}
    </div>
...
export default function ApprovalCenter() {
  ...
  const filtered = useMemo(() => items.filter(item => {
    ...
    if (filters.quick === 'eligible' && !item.bulkEligible) return false;
  ...
  return <div className="space-y-5 pb-12 text-slate-900 dark:text-slate-100">
    ...
    <div className="grid gap-5 [&>aside]:hidden [&>aside]:xl:block xl:grid-cols-[minmax(0,1fr)_310px]">
      <div className="space-y-3">{groups.map(group => {
        const standardRequests = group.items.filter(item => item.bulkEligible);
        const exceptions = group.items.filter(needsIndividualReview);
        const checked = standardRequests.filter(item => selected.has(item.canonicalKey));
        ...
        return <section key={group.kind} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-600 dark:bg-slate-800">
          <div className="flex flex-wrap items-center gap-3 p-4">
            ...
            {individualOnly ? <button onClick={() => setExpanded(group.kind)} className="min-h-11 rounded-lg bg-indigo-600 px-4 py-2 font-semibold text-white">Review queue</button> : <Button variant="success" disabled={!standardRequests.length} onClick={() => openConfirm(group.kind, standardRequests.map(item => item.id))}>Approve standard — {standardRequests.length}</Button>}
          </div>

...
const humanizeStatus = (status: string) => status.replace(/^WFH_/, '').replace(/_/g, ' ').replace(/([a-z])([A-Z])/g, '$1 $2').toLowerCase().replace(/\b\w/g, letter => letter.toUpperCase());
const contextNumber = (item: ApprovalItem, key: string) => {
  const value = item.approvalContext?.[key];
...
const ApprovalMobileCard: React.FC<{
  ...
  const totalWeekHours = contextNumber(item, 'totalWeekHours');
  const weekOtHours = contextNumber(item, 'weekOtHours');
  const monthlyWfhDays = contextNumber(item, 'monthWfhDays');
  const yearLeaveDays = contextNumber(item, 'yearLeaveDays');
  const threshold = contextNumber(item, 'threshold');
  const classification = item.exception ? 'Exception — individual review required' : item.route === 'BOD_REQUIRED' ? 'Additional BOD approval required' : item.bulkEligible ? 'Standard request' : 'Individual review';
  ...
  return <article className={`rounded-xl border p-4 shadow-sm ${requested ? 'border-indigo-400 bg-indigo-50 ring-1 ring-indigo-300 dark:bg-indigo-950' : 'border-slate-200 bg-white dark:border-slate-600 dark:bg-slate-800'}`}>
    <div className="flex items-start justify-between gap-3">
      ...
      {item.bulkEligible && <label className="flex min-h-11 flex-shrink-0 items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-200"><input type="checkbox" aria-label={`Select ${item.reference} for bulk approval`} disabled={!!item.exception} checked={selected} onChange={event => onSelect(event.target.checked)} /> Select</label>}
    </div>
    ...
    <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
      ...
      {item.details && <div className="col-span-2"><dt className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Details</dt><dd className="mt-0.5 break-words">{item.details}</dd></div>}
      {item.kind === 'overtime' && weekOtHours !== undefined && <div><dt className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Weekly OT</dt><dd className="mt-0.5 font-medium">{weekOtHours} hours</dd></div>}
      {item.kind === 'overtime' && totalWeekHours !== undefined && <div><dt className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Weekly total</dt><dd className="mt-0.5 font-medium">{totalWeekHours} hours{threshold !== undefined ? ` / ${threshold}` : ''}</dd></div>}
      {item.kind === 'wfh' && monthlyWfhDays !== undefined && <div className="col-span-2"><dt className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Monthly WFH</dt><dd className="mt-0.5 font-medium">{monthlyWfhDays} day{monthlyWfhDays === 1 ? '' : 's'}{threshold !== undefined ? ` / ${threshold}-day threshold` : ''}</dd></div>}
      {item.kind === 'leave' && yearLeaveDays !== undefined && <div className="col-span-2"><dt className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Year leave context</dt><dd className="mt-0.5 font-medium">{yearLeaveDays} day{yearLeaveDays === 1 ? '' : 's'}{threshold !== undefined ? ` / ${threshold}-day threshold` : ''}</dd></div>}
    </dl>
    <div className="mt-4 flex flex-wrap items-center gap-2"><span className="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-800 dark:bg-amber-950 dark:text-amber-200">{humanizeStatus(item.status)}</span><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${item.exception || item.route === 'BOD_REQUIRED' ? 'bg-violet-50 text-violet-700 dark:bg-violet-950 dark:text-violet-200' : 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-200'}`}>{classification}</span></div>
...
export default function ApprovalCenter() {
  ...
  useEffect(() => {
    ...
    const quick = requestedReview === 'eligible' || requestedReview === 'exceptions' ? requestedReview : undefined;
    if (kind || quick) {
  ...
  return <div className="space-y-5 pb-12 text-slate-900 dark:text-slate-100">
    ...
    <div className="grid gap-5 [&>aside]:hidden [&>aside]:xl:block xl:grid-cols-[minmax(0,1fr)_310px]">
      <div className="space-y-3">{groups.map(group => {
        ...
        return <section key={group.kind} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-600 dark:bg-slate-800">
          ...
          {expanded === group.kind && <div className="border-t border-slate-200 dark:border-slate-600">
            ...
            <div className="hidden overflow-x-auto lg:block"><table className="min-w-full text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500 dark:bg-slate-700 dark:text-slate-200"><tr>{['', 'Request / Employee', 'Business unit / Department', 'Submitted / Aging', 'Current step', 'Status', 'Classification', ''].map((heading, index) => <th key={index} className="px-4 py-3">{heading}</th>)}</tr></thead><tbody>{group.items.map(item => <tr id={`approval-${item.id}`} key={item.canonicalKey} className={`border-t border-slate-200 dark:border-slate-600 ${requestedItem === item.id ? 'bg-indigo-50 ring-1 ring-inset ring-indigo-300 dark:bg-indigo-950' : ''}`}><td className="px-4 py-3">{item.bulkEligible && <input type="checkbox" aria-label={`Select ${item.reference} for bulk approval`} disabled={!!item.exception} checked={selected.has(item.canonicalKey)} onChange={event => { const next = new Set(selected); event.target.checked ? next.add(item.canonicalKey) : next.delete(item.canonicalKey); setSelected(next); }} />}</td><td className="px-4 py-3 font-semibold"><div className="text-xs font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-300">{item.reference}</div>{item.employee}<div className="text-xs font-normal text-slate-500 dark:text-slate-300">{item.employeeCode || item.details || 'No employee ID'}</div>{item.employeeCode && item.details && <div className="text-xs font-normal text-slate-500 dark:text-slate-300">{item.details}</div>}</td><td className="px-4 py-3">{item.businessUnit}<div className="text-xs text-slate-500 dark:text-slate-300">{item.department}</div></td><td className="whitespace-nowrap px-4 py-3">{fmtDate(item.start)}<div className={`text-xs ${dayAge(item.start) >= 3 ? 'font-semibold text-red-600 dark:text-red-300' : 'text-slate-500 dark:text-slate-300'}`}>{dayAge(item.start)} day{dayAge(item.start) === 1 ? '' : 's'} pending · {item.duration}</div></td><td className="px-4 py-3">{item.currentStep}</td><td className="px-4 py-3"><span className="rounded-full bg-amber-50 px-2 py-1 text-amber-700">{humanizeStatus(item.status)}</span></td><td className="px-4 py-3">{item.exception ? <span className="font-semibold text-amber-700 dark:text-amber-300">Needs individual review<div className="font-normal">{item.exception}</div></span> : item.route === 'BOD_REQUIRED' ? <span className="font-semibold text-violet-700 dark:text-violet-300">Additional BOD approval required</span> : <span className="text-emerald-700 dark:text-emerald-300">{item.bulkEligible ? 'Standard request' : 'Individual review'}</span>}</td><td className="px-4 py-3"><Link className="inline-flex min-h-11 items-center font-semibold text-indigo-600 dark:text-indigo-300" to={item.reviewUrl}>Review request →</Link></td></tr>)}</tbody></table></div>
          </div>}
        </section>;

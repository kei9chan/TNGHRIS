...
const PayrollStaging: React.FC = () => {
    ...
    const generatePayrollData = () => {
        ...
        const relevantUsers = mockUsers.filter(u => u.role === Role.Employee);

        const data: PayrollStagingRecord[] = relevantUsers.map(employee => {
            const attendance = mockAttendanceRecords.filter(r => 
                r.employeeId === employee.id && 
                new Date(r.date) >= start && 
                new Date(r.date) <= end
            );
            const overtime = mockOtRequests.filter(r => 
                r.employeeId === employee.id && 
                r.status === 'Approved' &&
                new Date(r.date) >= start && 
                new Date(r.date) <= end
            ...
            const overtimeHours = overtime.reduce((sum, r) => sum + (r.approvedHours || 0), 0);
            
            const allowances = 0; // Default
    ...
    useEffect(generatePayrollData, [startDate, endDate]);
    ...
    const handleGeneratePayslips = () => {
        const newPayslips: PayslipRecord[] = payrollData.map(rec => ({
            ...
            netPay: rec.netPay,
            // FIX: Add missing 'status' and 'lastGenerated' properties to satisfy the PayslipRecord type.
            status: 'draft',
            lastGenerated: new Date(),
            earningsBreakdown: {
                regularPay: rec.regularHours * REGULAR_RATE,
                otPay: rec.overtimeHours * OT_RATE,
    ...
    return (
        <div className="space-y-6">
             <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Payroll Staging</h1>
                    <p className="text-gray-600 dark:text-gray-400 mt-1">
                        Generate, review, and finalize payroll data before processing.
                    </p>
                </div>
            </div>
            ...
            <Card>
                 <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-4 p-4 items-end">
                    ...
                    <Button onClick={generatePayrollData} className="w-full">Generate</Button>
                     {canManage && (
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-700">
                           <tr>
                                ...
                                <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">OT Hrs</th>
                                <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Allowances</th>
                                <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Deductions</th>
                           ...
                           {payrollData.map(rec => (
                               <tr key={rec.id} className={editModes[rec.id] ? 'bg-indigo-50 dark:bg-indigo-900/10' : ''}>
                                   ...
                                   <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                        {editModes[rec.id] ? (
                                            <input type="number" value={editData.allowances ?? rec.allowances} onChange={e => handleEditDataChange('allowances', e.target.value)} className="w-24 p-1 rounded-md border-gray-300 dark:bg-gray-700" />
                                        ) : `$${rec.allowances.toFixed(2)}`}
                                   </td>
                                   <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                        {editModes[rec.id] ? (
                                             <input type="number" value={editData.deductions ?? rec.deductions} onChange={e => handleEditDataChange('deductions', e.target.value)} className="w-24 p-1 rounded-md border-gray-300 dark:bg-gray-700" />

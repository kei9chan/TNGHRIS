...
export function useAttendanceClock(){
 ...
 const [elapsed,setElapsed]=useState(0);const refreshing=useRef(0);const inFlight=useRef(false);const generation=useRef(0);const anchor=useRef({server:0,elapsed:0,tick:0,state:'not_started'});
 ...
 const refresh=useCallback(async()=>{if(!user)return;const stamp=++generation.current;refreshing.current++;try{const value=await getMyAttendance();if(stamp===generation.current){accept(value);setError('');}}catch(e){if(stamp===generation.current)setError((e as Error).message);}finally{refreshing.current--;if(stamp===generation.current)setLoading(false);}},[user?.id,accept]);
 useEffect(()=>{setDay(null);setLoading(true);void refresh();const timer=setInterval(()=>{if(document.visibilityState==='visible'&&!inFlight.current&&!refreshing.current)void refresh();},30000);
 const onFocus=()=>{if(document.visibilityState==='visible'&&!inFlight.current&&!refreshing.current)void refresh();};window.addEventListener('focus',onFocus);window.addEventListener('online',onFocus);window.addEventListener('attendance-updated',onFocus);document.addEventListener('visibilitychange',onFocus);

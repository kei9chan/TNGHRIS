...
const record={id:'11111111-1111-4111-8111-111111111111',employeeId:'22222222-2222-4222-8222-222222222222',employeeName:'Fixture',effectiveDate:new Date(2026,8,7),templateVersion:2,templateName:'Fixture template',templateSnapshot:{name:'Fixture'},actionTaken:{salaryIncrease:true},particulars:{from:{salary:{basic:'PHP 18,128'}},to:{salary:{basic:20000,deminimis:1000,reimbursable:0}}},routingSteps:[{userId:'33333333-3333-4333-8333-333333333333'}]};
const p=panPayload(record,'44444444-4444-4444-8444-444444444444');assert.equal(p.id,record.id);assert.equal(p.effective_date,'2026-09-07');assert.equal(p.particulars.from.salary.basic,18128);assert.equal(p.particulars.to.salary.reimbursable,0);assert.equal(p.particulars.panTemplate.version,2);assert.deepEqual(p.routing_steps,record.routingSteps);
...
const modal=await readFile('components/employees/PANModal.tsx','utf8');assert.match(modal,/saveLock.current \|\| sent/);assert.match(modal,/error.savedPan.id/);assert.match(modal,/role="alert"/);assert.match(modal,/disabled=\{saving \|\| sent\}/);

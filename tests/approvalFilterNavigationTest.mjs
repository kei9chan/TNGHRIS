...
for(const kind of scope.GROUP_ORDER){enter('type='+kind);assert.equal(filters.kind,kind);assert.equal(expanded,kind);assert.equal(filters.search,'');assert.equal(filters.quick,'all');}
...
console.log('PASS: fresh visits clear stale filters; explicit category links reset unrelated filters; Show all clears category URL and selection.');

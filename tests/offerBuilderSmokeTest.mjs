...
const logoService = fs.readFileSync('services/businessUnitLogoService.ts', 'utf8');
...
assert.match(logoService, /file\.size > MAX_BYTES/);
assert.match(logoService, /MAX_BYTES = 2 \* 1024 \* 1024/);
assert.match(logoService, /MAX_EDGE = 4096/);
assert.match(logoService, /image\/webp/);
assert.match(builder, /saveBusinessUnitLogo\(selectedBusinessUnitId, file\)/);
assert.match(builder, /business-unit logo/i);
...
assert.match(offerWorkspace, /requireConnectedGmail/);
assert.match(offerWorkspace, /sendHrisEmail/);
assert.doesNotMatch(offerWorkspace, /send-recruitment-email|\/api\/recruitment-email/);
assert.match(offerWorkspace, /emailDelivery/);
assert.match(offerWorkspace, /status: draft\.status === OfferStatus\.Draft \? OfferStatus\.Sent : draft\.status/);
assert.match(offerWorkspace, /Published offer content cannot be edited/);

...
destinations.push({id:'bu-gootopia-second',name:'Gootopia Second Branch',brand:'Gootopia',color:'#7c3aed'});
...
assert.equal(await page.getByRole('article',{name:'Bakebe',exact:true}).count(),0);
await page.getByRole('article',{name:'The Dessert Museum',exact:true}).getByRole('button',{name:'Request Visit'}).click();
assert.equal(await page.getByLabel('Choose a location').count(),0);
await page.getByRole('article',{name:'Gootopia',exact:true}).getByRole('button',{name:'Request Visit'}).click();
await page.getByLabel('Choose a location').selectOption('bu-1');
...
assert.equal(await page.getByRole('article',{name:'Gootopia',exact:true}).getByRole('button',{name:'Request Visit'}).count(),0);

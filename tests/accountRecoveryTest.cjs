...
(async()=>{
 ...
 const failed=await scenario({sendFailure:true});assert.equal(failed.body,ok.body);assert(failed.updates.some(x=>x.failure_code==='gmail_send_failed'));
 const outage=await scenario({senderFailure:true});assert.equal(outage.status,503);assert.equal(outage.generated,0);

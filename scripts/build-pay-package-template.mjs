...
const wb=Workbook.create();const guide=wb.worksheets.add('Instructions'),input=wb.worksheets.add('Data Entry'),examples=wb.worksheets.add('Examples — Do Not Upload');
const required=new Set(['Employee code','Business unit','Pay type','Effective date','Amount unit','Approved basic pay / fee','Salary source','Reason for this record']);
...
const rows=[sample('DEMO-001','Employee salary',25000,mode[0],null,'Meal allowance',1000),sample('DEMO-002','Employee salary',30000,mode[1],30000),sample('DEMO-003','Employee salary',30000,mode[2],30000),sample('DEMO-004','Employee salary',30000,mode[0],null),sample('DEMO-004','Consultant fee',15000,mode[0],null,'Project completion fee',5000)];
rows[4][15]='One time';rows[4][16]=new Date('2026-12-15T00:00:00Z');
for(const s of [input,examples]){
 const count=s===input?101:6;
 s.getRangeByIndexes(0,0,1,headers.length).values=[headers.map(h=>h+(required.has(h)?'*':' (optional)'))];
 ...
 for(const [i,h] of headers.entries())s.getCell(0,i).format.fill=required.has(h)?'#6D28D9':'#64748B';
 s.getRangeByIndexes(1,0,count-1,1).setNumberFormat('@');
...
const fieldHelp={
'Employee code':['Required','Copy from Employee codes you can access on the upload page. Do not invent an ID. Missing employee? Register in HRIS first.','DEMO-001 (example only)'],
...
const info=[
 ['Template type and version','pay-packages:1'],
 ['START HERE','Fill Data Entry only. Required fields have *. Keep IDs as text; DEMO IDs are rejected. Paste values, never formulas. Field Guide explains EVERY field and gives exact choices/examples. Use Examples as a guide; replace sample details with approved facts. Upload on Payroll → Pay Packages, review the preview, then submit reviewed rows for approval. Never relabel a net agreement as Gross.'],
 ['1. Use Data Entry','One continuous row contains employee, salary, net/gross agreement and allowances. No package keys. Examples and this Guide are NOT imported.'],
 ...
 ['13. Import and check','Payroll → Pay Packages → upload. Check every preview row before submission. Importing neither approves pay nor releases payment. Old two-tab workbooks are still supported.'],
...
console.log((await wb.inspect({kind:'table',range:"'Examples — Do Not Upload'!A1:M6",tableMaxRows:6,tableMaxCols:13,maxChars:2400})).ndjson);

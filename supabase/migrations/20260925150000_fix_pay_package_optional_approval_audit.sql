-- Approval comments are optional. Keep the audit row complete with a
-- system-generated reason when an approver intentionally leaves the note blank.
create or replace function public.review_payroll_pay_package(p_package_id uuid,p_approve boolean,p_reason text)
returns void
language plpgsql
security definer
set search_path=''
...
begin
  ...
  if p.source_kind='approved_pan' then
    raise exception 'PAN approval is the compensation approval. Create a correction instead of reapproving or editing this package.';
  end if;
  if p.status<>'draft' or p.approval_state<>'pending' then
    raise exception 'This package is not pending compensation approval.';
  end if;
  if actor is null or actor_auth is null then
    raise exception 'Authenticated reviewer required.' using errcode='42501';
  end if;
  if p.created_by=actor_auth or p.created_by=actor then
    raise exception 'The creator cannot approve their own compensation entry.' using errcode='42501';
  end if;
  if not p_approve and length(coalesce(decision_note,''))<3 then
    raise exception 'A rejection reason of at least 3 characters is required.';
  end if;
  audit_reason:=coalesce(decision_note,case when p_approve then 'Approved without comment' else null end);
  ...
  if not p_approve then
    ...
    values(p.employee_id,p.scope_id,p.id,actor_auth,'reject',audit_reason,to_jsonb(p),(select to_jsonb(x) from public.payroll_pay_packages x where x.id=p.id),p.source_kind,'pay-package-builder-v2');
    return;
  end if;
  ...
  insert into public.payroll_pay_audit(employee_id,scope_id,package_id,actor_id,action,reason,previous_value,new_value,source,calculation_version)
  values(p.employee_id,p.scope_id,p.id,actor_auth,case when all_done then 'approve_and_activate' else 'approval_step' end,audit_reason,to_jsonb(p),(select to_jsonb(x) from public.payroll_pay_packages x where x.id=p.id),p.source_kind,'pay-package-builder-v2');
end $$;

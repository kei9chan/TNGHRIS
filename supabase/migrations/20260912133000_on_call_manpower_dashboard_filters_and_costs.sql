-- Dashboard v2: expose the remaining filters and make actual cost totals
-- include verified regular attendance plus verified replacement cost.  The
-- original RPC remains intact for backwards compatibility; this wrapper
-- applies additional filtering to the already scope-restricted JSON result.
...
begin
  ...
  select coalesce(jsonb_agg(
    day_row || jsonb_build_object(
      'actualCost',
        coalesce((day_row->>'regularCost')::numeric, 0)
  ...
  select jsonb_build_object(
    ...
    'totalOnCallHours', coalesce((
      select sum((employee_row->>'onCallHours')::numeric)
      from jsonb_array_elements(filtered_employees) as employee_row
    ), 0),
    ...
    'reportingRate', case
      when coalesce(sum((day_row->>'scheduled')::numeric), 0) = 0 then 0
      else round(100 * coalesce(sum((day_row->>'reported')::numeric), 0)
        / sum((day_row->>'scheduled')::numeric), 1)
    end
  )

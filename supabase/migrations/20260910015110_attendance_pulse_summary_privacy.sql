-- Summaries expose only generic HR review completion, never NTE draft/case stages.
...
create or replace function attendance_pulse.read(p_actor uuid,p_date date) returns jsonb language plpgsql stable security definer set search_path='' as $$declare d jsonb;patterns jsonb;alerts jsonb;rows0 jsonb;attention integer;begin
 ...
 select coalesce(jsonb_agg(x||jsonb_build_object('hrState',case when c.state like 'closed%' then 'closed' end,'needsAttention',coalesce((x->>'needsAttention')::boolean,false) and coalesce(c.state not like 'closed%',true)) order by x->>'submittedAt' desc),'[]') into rows0 from jsonb_array_elements(d->'rows') x left join attendance_issues.hr_cases c on c.request_id=(x->>'id')::uuid;

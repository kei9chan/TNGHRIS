-- Interview scheduling and ATS action metadata.
-- This migration is additive: existing applications, candidates, and interviews
-- remain intact while the scheduler can persist calendar/invite outcomes.

alter table public.job_interviews
  add column if not exists panel_user_ids uuid[] not null default '{}',
...
alter table public.job_applications
  ...
  add column if not exists rejection_email_subject text;
...
create index if not exists job_applications_rejected_at_idx
  on public.job_applications (rejected_at);

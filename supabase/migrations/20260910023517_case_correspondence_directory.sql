...
declare actor uuid:=public.current_hris_user_id(); full_view boolean; r public.incident_reports; result jsonb; q record;
begin
 ...
 for q in select id from case_correspondence.questions where case_id=p_id and recipient=actor and opened_at is null for update loop update case_correspondence.questions set opened_at=clock_timestamp() where id=q.id;end loop;

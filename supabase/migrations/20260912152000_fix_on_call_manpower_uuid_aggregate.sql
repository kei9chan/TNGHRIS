-- Fix PostgreSQL's lack of max(uuid) support in the employee aggregate.
-- Keep the stable UUID representative without changing dashboard scope or RLS behavior.
...
begin
  ...
  with scheduled as (
    ...
    select s.employee_id, max(s.full_name) employee_name, (array_agg(s.business_unit_id order by s.date desc nulls last))[1] business_unit_id,

-- Proposals are separate from effective assignments; approval uses the existing
-- schedule writer, publication validation, freeze guards and immutable audit.
create table schedule_compliance.submissions (
 ...
 version integer not null default 1, entries jsonb not null, template_hash text not null, draft_hash text not null,
 status text not null check(status in('Pending','Approved','Rejected')), reason text not null,
 reviewed_by uuid references public.hris_users(id), review_reason text, submitted_at timestamptz not null default now(), reviewed_at timestamptz,
 unique(employee_id,week)
);
alter table schedule_compliance.submissions enable row level security;
revoke all on schedule_compliance.submissions from public,anon,authenticated;
create trigger submission_audit after insert or update or delete on schedule_compliance.submissions for each row execute function schedule_compliance.audit_row();

create function schedule_compliance.bod_manager(p_employee uuid) returns uuid language sql stable security definer set search_path='' as $$
 select b.id from public.hris_users e join public.hris_users b on b.id::text=e.reports_to
 where e.id=p_employee and e.id<>b.id and lower(e.status::text)='active' and private.workflow_user_has_role(b.id,'Board of Director');
$$;
create function schedule_compliance.template_hash(p_entries jsonb) returns text language sql stable security definer set search_path='' as $$
 select md5(coalesce(jsonb_agg(to_jsonb(t) order by t.id)::text,'[]')) from public.shift_templates t
 where t.id in(select (x->>'templateId')::uuid from jsonb_array_elements(p_entries) x where x->>'templateId' is not null);
$$;

create function public.get_bod_schedule_workflow(p_week date default null) returns jsonb language plpgsql security definer set search_path='' as $$
declare actor uuid:=private.payroll_actor_id();m uuid;w date:=coalesce(p_week,date_trunc('week',now() at time zone 'Asia/Manila')::date+7);own jsonb;templates jsonb:='[]';task jsonb;pending jsonb;begin
 if actor is null then raise exception 'Active account required' using errcode='42501';end if;
 ...
 select jsonb_agg(x||jsonb_build_object('name',coalesce(t.name,'Keep existing schedule / approved status'),'start',t.start_time,'end',t.end_time) order by x->>'date')
...
create function public.submit_my_bod_schedule(p_week date,p_entries jsonb,p_reason text) returns uuid language plpgsql security definer set search_path='' as $$
declare actor uuid:=private.payroll_actor_id();m uuid;bu uuid;x jsonb;t public.shift_templates;d date;r schedule_compliance.submissions;canonical jsonb;begin
 perform pg_advisory_xact_lock(hashtextextended('payroll-schedule-publication',0));
 m:=schedule_compliance.bod_manager(actor);
 if actor is null or m is null then raise exception 'Only active employees reporting directly to a BOD can submit their own schedule here' using errcode='42501';end if;
 ...
 if (select count(distinct x->>'date') from jsonb_array_elements(p_entries) x)<>7 then raise exception 'Each day must appear once';end if;
 ...
 if private.schedule_day_status(actor,d)->>'tag' in('suspended','absence','company_holiday','rest','skeletal') then raise exception 'Keep existing HR schedule statuses; contact HR to change them';end if;
 ...
 select jsonb_agg(jsonb_build_object('date',x->>'date','templateId',x->>'templateId') order by x->>'date') into canonical from jsonb_array_elements(p_entries) x;
...
create function public.review_bod_schedule_submission(p_id uuid,p_version integer,p_approve boolean,p_reason text) returns void language plpgsql security definer set search_path='' as $$
declare actor uuid:=private.payroll_actor_id();r schedule_compliance.submissions;x jsonb;bu uuid;dept uuid;draft jsonb;begin
 perform pg_advisory_xact_lock(hashtextextended('payroll-schedule-publication',0));
 select * into r from schedule_compliance.submissions where id=p_id for update;
 if actor is null or r.id is null or actor=r.employee_id or actor is distinct from schedule_compliance.bod_manager(r.employee_id) or r.manager_id<>actor then raise exception 'Only the assigned BOD may review this schedule' using errcode='42501';end if;
 ...
 for x in select value from jsonb_array_elements(r.entries) where value->>'templateId' is not null loop
...
-- Suppress both future deliveries and already queued/retry deliveries to BODs,
-- including multi-role BOD/Admin/HR users. Keep historical sent records intact.
create function schedule_compliance.suppress_bod_email() returns trigger language plpgsql security definer set search_path='' as $$begin
 if new.status in('pending','failed','sending') and private.workflow_user_has_role(new.recipient_id,'Board of Director') then
 new.status:='skipped';new.error:='BOD employee-schedule emails disabled';new.lease:=null;new.lease_until:=null;
 end if;return new;
end $$;
create trigger suppress_bod_schedule_email before insert or update on schedule_compliance.deliveries for each row execute function schedule_compliance.suppress_bod_email();
update schedule_compliance.deliveries set status='skipped',error='BOD employee-schedule emails disabled',lease=null,lease_until=null,updated_at=clock_timestamp()
 where status in('pending','failed','sending') and private.workflow_user_has_role(recipient_id,'Board of Director');
create function public.schedule_email_recipient_allowed(p_id uuid) returns boolean language sql stable security definer set search_path='' as $$
 select exists(select 1 from schedule_compliance.deliveries d join public.hris_users h on h.id=d.recipient_id where d.id=p_id and d.status='sending' and lower(h.status::text)='active' and not private.workflow_user_has_role(h.id,'Board of Director'));
$$;
revoke all on function schedule_compliance.bod_manager(uuid),schedule_compliance.template_hash(jsonb),schedule_compliance.suppress_bod_email() from public,anon,authenticated;
revoke all on function public.get_bod_schedule_workflow(date),public.submit_my_bod_schedule(date,jsonb,text),public.review_bod_schedule_submission(uuid,integer,boolean,text),public.schedule_email_recipient_allowed(uuid) from public,anon,authenticated;
...
grant execute on function public.schedule_email_recipient_allowed(uuid) to service_role;

...
declare today date:=(statement_timestamp() at time zone 'Asia/Manila')::date;w date;m record; s jsonb;ev text;hr record;counted integer:=0;t record;begin
 ...
 for t in select queued.* from schedule_compliance.tasks queued join public.hris_users h on h.id=queued.manager_id where lower(h.status::text)='active' and queued.completed_at is null loop

...
begin
 ...
 for h in select distinct a.ob_id,a.employee_id,(a.approval_snapshot->>'work_date')::date as work_date,r.reference
 from public.official_business_punch_audits a join public.official_business_requests r on r.id=a.ob_id
 ...
 and (r.approved_at is null or r.status in('Cancelled','Rejected')
 or a.approval_snapshot->>'work_date' is distinct from r.work_date::text
 or (a.approval_snapshot->>'starts_at')::timestamptz is distinct from r.starts_at
 or (a.approval_snapshot->>'ends_at')::timestamptz is distinct from r.ends_at
 or (a.approval_snapshot->>'latitude')::double precision is distinct from r.latitude
 or (a.approval_snapshot->>'longitude')::double precision is distinct from r.longitude
 or (a.approval_snapshot->>'radius_metres')::integer is distinct from r.radius_metres
 or (a.approval_snapshot->>'override_schedule')::boolean is distinct from r.override_schedule) loop

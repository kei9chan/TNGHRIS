...
import NTEDeductionPanel from './NTEDeductionPanel';
...
const NoticeDecision:React.FC<{nteId:string;refresh:number}>=({nteId,refresh})=>{
 const[w,setW]=useState<any>(null),[form,setForm]=useState<Record<string,any>>({}),[error,setError]=useState(''),[busy,setBusy]=useState(false),[version,setVersion]=useState(0),[reason,setReason]=useState(''),[dates,setDates]=useState(''),[rtw,setRtw]=useState('');
 ...
 return <Card title="Notice of Decision">
  ...
  {!d?<p>No issued decision. Management drafting opens after the response window closes.</p>:<div className="space-y-4">
   ...
   {d.resolution_type==='Salary Deduction'&&<NTEDeductionPanel nteId={nteId} refresh={version}/>}

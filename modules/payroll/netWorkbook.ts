...
const fields=['employeeId','employeeName','sssBase','philhealthBase','pagibigBase','sssCovered','philhealthCovered','pagibigCovered','coverageRef','openingTaxable','openingWithheld','openingPeriods','previousEmployer','cumulativeAlready','sourceRef','openingRef',...contributionKeys,'voluntaryPercent','higherDeductionAuthorization','higherDeductionLegalBasis'] as const;
...
export async function downloadNetWorkbook(w:NetWorkspace,p:NetInputs){
 ...
 ['Other deductions','Enter only authorized non-loan deductions. An empty sheet confirms none for this cutoff. kind: legal or voluntary. Statutory then legal deductions, loans, other voluntary. Default 20% voluntary ceiling after statutory deductions. carryForward yes requires authorization in sourceRef. Prior authorized deferred items carry automatically; do not enter them twice.'],
 ...
 const deductions=book.addWorksheet('Other deductions');deductions.addRow(['employeeId','label','amount','sourceRef','kind','carryForward']);p.employees.forEach(e=>e.deductions.forEach(d=>deductions.addRow([e.employeeId,d.label,d.amount,d.sourceRef,d.kind||'voluntary',d.carryForward?'yes':'no'])));
...
export async function importNetWorkbook(file:File,w:NetWorkspace,current:NetInputs):Promise<{inputs:NetInputs;loans:Loan[]}>{
 ...
 const employeeRows=rows('Employee review',fields);const taxRows=rows('Tax lines',['employeeId','employeeName','lineIndex','label','gross','taxable','kind','exemptionRef']);const deductionRows=rows('Other deductions',['employeeId','label','amount','sourceRef','kind','carryForward']);const loanRows=rows('Loan balances',loanFields);
 ...
 const employees=employeeRows.map(row=>{if(!ids.has(row.employeeId)||seen.has(row.employeeId))throw new Error('Unknown or duplicate employee review.');seen.add(row.employeeId);const e={...current.employees.find(x=>x.employeeId===row.employeeId),...row,openingContributions:Object.fromEntries(contributionKeys.map(k=>[k,row[k]])),taxLines:[],deductions:[]} as unknown as NetEmployee;
 ...
 e.deductions=deductionRows.filter(d=>d.employeeId===row.employeeId).map(d=>({label:d.label,amount:d.amount,sourceRef:d.sourceRef,kind:d.kind||'voluntary',carryForward:d.carryForward.toLowerCase()==='yes'}));return e;});

...
export type ComponentCategory = 'de_minimis'|'fixed_allowance'|'reimbursable_allowance'|'service_charge'|'variable_pay'|'employee_deduction'|'employee_paid_benefit'|'employer_paid_benefit'|'employer_contribution'|'other';
export type Responsibility = 'employee'|'employer'|'split';
export type ReceiptStatus = 'receipt_required'|'receipt_submitted'|'under_review'|'approved_and_payable'|'rejected'|'not_payable';
export type PayComponent = {
 tax?:string;sss?:string;philhealth?:string;pagibig?:string;thirteenthMonth?:string;proration?:string;
 name: string;
 amount: string;
 recurrence: string;
 legacyField: string;
...
export type PayPackageDocument = {id:string;path:string;name:string;uploadedAt:string};
export type PayPackage = {
 ...
 status: string; source_ref: string; reason: string; approved_at: string | null; source_pan_id: string | null; documents?:PayPackageDocument[];
};
export type PayScope = {id:string;name:string;businessUnitId?:string|null;canEdit:boolean;canApprove:boolean;employeePayroll?:boolean};
export type PayDirectoryEntry = {id:string;name:string;employeeCode:string;businessUnit?:string;businessUnitId?:string|null;department?:string;departmentId?:string|null;status?:string};
...
export const fetchPayDirectory = () => rpc<PayDirectoryEntry[]>('get_payroll_package_directory');
...
export async function uploadPayPackageDocument(packageId:string,file:File){
 const safe=file.name.replace(/[^a-zA-Z0-9._-]+/g,'-').slice(-180)||'supporting-document';
 const path=`${packageId}/${crypto.randomUUID()}-${safe}`;
 const {error}=await supabase.storage.from('payroll-pay-package-documents').upload(path,file,{upsert:false,contentType:file.type||undefined});
 if(error)throw new Error(`Supporting document upload failed: ${error.message}`);
 try{return await rpc<string>('attach_payroll_pay_package_document',{p_package_id:packageId,p_path:path,p_name:file.name});}
 catch(error){await supabase.storage.from('payroll-pay-package-documents').remove([path]);throw error;}
}

export async function openPayPackageDocument(path:string){
 const {data,error}=await supabase.storage.from('payroll-pay-package-documents').createSignedUrl(path,60);
 if(error||!data?.signedUrl)throw new Error(error?.message||'Supporting document could not be opened.');
 window.open(data.signedUrl,'_blank','noopener,noreferrer');
}

export const emptyTreatment = (): Treatment => ({
 ...Object.fromEntries(treatmentFields.map(([key])=>[key,'unreviewed'])),
 payBasis:'gross',taxResponsibility:'employee',taxCoverage:'entire_package',benefitResponsibility:'employee',calculationVersion:'pay-package-builder-v1'
});
export const newComponent = (category:ComponentCategory='fixed_allowance'): PayComponent => ({
 ...emptyTreatment(),name:'',amount:'',recurrence:'recurring',legacyField:'',payableDate:'',category,frequency:'Monthly',paidBy:'employer',taxTreatment:category==='de_minimis'?'non_taxable':category==='reimbursable_allowance'?'reimbursable':'taxable',employeeShare:'0',employerShare:'0',companyPaidEmployeeShare:'0',effectiveDate:'',eligibilityRule:'All eligible employees',policyRef:'',policyLimit:'',receiptRequired:category==='reimbursable_allowance',receiptStatus:category==='reimbursable_allowance'?'receipt_required':undefined,documentRef:'',status:'active'
});

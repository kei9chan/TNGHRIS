...
export type Approval={id:string;scopeId:string;step:number;stage:string;current:boolean;returned:boolean;paid:boolean;mode:string;canAct:boolean;canDisburse:boolean;staleReason:string|null;reference:string;contact:string;source:{kind:string;version:number;from:string;to:string;payDate:string;gross:string;deductions:string;net:string;employees:{employeeId:string;employeeName:string;gross:string;deductions:string;net:string;tax:string;lines:{label:string;amount:string;remaining?:string}[]}[]};actions:{step:number;stage:string;action:string;actor:string;at:string;reason:string}[]};

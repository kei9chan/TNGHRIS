...
export async function buildPayslipPdf(s:EmployeePayslip){
 ...
 line(`Payment: ${s.paymentStatus||'Released after payment'}`);line(`Amount still due: PHP ${Number(s.unpaid||0).toFixed(2)}`);y+=4;
 for(const g of payslipGroups(s)){line(g.title,true);for(const l of g.lines){line(`${l.label}: PHP ${Number(l.remaining??l.amount??0).toFixed(2)}`);const info=[l.date&&`Date ${l.date}`,l.quantity!=null&&`Quantity ${l.quantity}`,l.rate!=null&&`Rate ${l.rate}`,l.factor!=null&&`Multiplier ${l.factor}`].filter(Boolean).join(' | ');if(info)line(info);if(l.deferred)line(`Deferred this cutoff: PHP ${Number(l.deferred).toFixed(2)}`);if(l.projectedBalance)line(`Remaining balance: PHP ${Number(l.projectedBalance).toFixed(2)}`);}y+=3;}

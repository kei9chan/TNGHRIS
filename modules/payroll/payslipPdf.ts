...
export async function buildStyledPayslipPdf(s: EmployeePayslipDocumentData, test = false) {
  ...
  const table=(title:string,rows:{label:string;amount:number;units?:string}[],totalLabel:string,total:unknown,x:number,w:number)=>{if(!rows.length)return;card(x,y,w,9,[242,238,255]);text(title.toUpperCase(),x+4,y+6,9,true,purple);let yy=y+14;rows.forEach(row=>{text(row.label,x+4,yy,8);if(row.units)text(row.units,x+w-40,yy,7,false,slate);right(amount(row.amount),x+w-4,yy,8,true);yy+=6;});pdf.setDrawColor(220,225,233);pdf.line(x+4,yy-2,x+w-4,yy-2);text(totalLabel,x+4,yy+4,8,true);right(amount(total),x+w-4,yy+4,9,true);return yy+8;};

...
export default function PhaseChecklist(){
 ...
 return <Card title="Your team's setup checklist">
  <p className="mb-3 text-sm">Choose a business unit to review or update team progress. Saved confirmations are shared across payroll; actual cutoff checks still determine readiness.</p>
  ...
  <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr>{['Phase','Software','Team status','Owner / task'].map(h=><th className="p-2" key={h}>{h}</th>)}</tr></thead><tbody>{phaseChecklist.map((p,i)=>{const saved=scope?.progress.find(s=>s.phase===i+1);return <tr key={p.phase} className="border-t dark:border-slate-700"><td className="p-2 align-top whitespace-nowrap">{p.link?<Link className="text-indigo-600 dark:text-indigo-300" to={p.link}>{p.phase}</Link>:p.phase}</td><td className="p-2 align-top">Implemented</td><td className="p-2 align-top min-w-48"><strong>{saved?statuses[saved.status]:(scope?'Not yet recorded':'Choose a BU')}</strong>{saved&&<><p>{saved.evidence}</p><p className="text-xs">Updated {new Date(saved.recorded_at).toLocaleDateString('en-PH')}</p></>}</td><td className="p-2 min-w-80"><p className="font-medium">{p.owner}</p><details className="mt-1"><summary className="cursor-pointer text-indigo-600 dark:text-indigo-300">View tasks</summary><p className="mt-2">{p.todo}</p></details></td></tr>;})}</tbody></table></div>

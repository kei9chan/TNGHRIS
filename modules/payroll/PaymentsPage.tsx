import {BankingPolicyPanel} from './ConfirmedPolicyPanels';
...
export default function PaymentsPage(){
 ...
 return <div className="space-y-6">
  ...
  <Card title="Existing payment and filing processes">
   ...
   {scope&&<BankingPolicyPanel key={scope.id} scopeId={scope.id}/>}
  ...
  {w&&<>
   <Card title="Payment reconciliation">
    ...
    <p className="mt-3 text-sm">Pending attempts reserve their amount. Reissue only a confirmed failure, cancellation or full return. Approved payslips appear by payday with payment status. Loan posting follows full-batch reconciliation; individual partial payments remain visible here.</p>
   ...
   {w.batch&&!w.batch.closed&&w.canRelease&&<Card title="Prepare an attempt or linked reissue"><p className="mb-3 text-sm">Record a distinct payment reference for this employee and transfer. Use the existing verified payment process to send it. Preparing an attempt does not confirm payment.</p>
    <form className="space-y-3" onSubmit={e=>{e.preventDefault();void perform(()=>preparePaymentAttempt(w.batch!.id,employee,amount,reference,date,reissue||null),'Attempt recorded as pending.');}}>
     ...
     <label>Amount (PHP)<input inputMode="decimal" className={input} value={amount} onChange={e=>setAmount(e.target.value)} required disabled={busy}/></label><label>Scheduled date<input type="date" className={input} value={date} onChange={e=>setDate(e.target.value)} required disabled={busy}/></label></div>

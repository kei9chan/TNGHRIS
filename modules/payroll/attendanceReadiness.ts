...
export type TimeRow={employeeId:string;employeeName:string;date:string;restDay:boolean;holiday:boolean;approvedFullLeave:boolean;scheduledMinutes:number;actualMinutes:number;regularMinutes:number;breakMinutes:number;lateMinutes:number;undertimeMinutes:number;approvedOtMinutes:number;actualOtMinutes:number;workedLunch:boolean;issues:string[];ready:boolean;shiftIds:string[];eventIds:string[];leaveIds:string[];ot:{id:string;type:string;status:string}[];segments:{date:string;start:string;end:string}[]};

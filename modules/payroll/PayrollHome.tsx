...
export default function PayrollHome(){
 ...
 <section className="rounded-2xl border bg-white p-5 dark:border-slate-700 dark:bg-slate-900"><div className="grid gap-4 md:grid-cols-3"><label className="font-medium">Business unit<select className={field} disabled={contextLoading} value={scopeId} onChange={e=>setScopeId(e.target.value)}><option value="">Choose an accessible business unit</option>{scopes.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label><label className="font-medium">Cutoff starts<input type="date" className={field} value={from} onChange={e=>setFrom(e.target.value)}/></label><label className="font-medium">Cutoff ends<input type="date" className={field} value={to} onChange={e=>setTo(e.target.value)}/></label></div><p className="mt-3 text-sm text-slate-500">Your selection is retained on this browser. For the pilot, choose a completed cutoff with reliable attendance and an existing payroll register.</p><button className="mt-4 min-h-11 rounded-lg border px-4 font-medium" disabled={loading||contextLoading} onClick={()=>setRefresh(v=>v+1)}>Refresh readiness</button></section>

...
export function prepareImport(row:ImportRow,context:PayContext){
 ...
 const existing=context.packages.find(p=>p.scope_id===scope!.id&&p.stream===stream&&p.effective_from===v['Effective from']&&p.status!=='rejected'&&p.status!=='superseded');
 if(existing?.status==='approved')fail('An approved package already exists for this employee, business unit, pay stream, and effective date. Create a dated update instead.');
 if(existing?.status==='draft'&&existing.approval_state==='pending')fail('This pay package has already been submitted and is pending approval. Track it under Pending for approval.');

...
function AttendanceWorkspace({scope,setScope,from,to}:{key?:string;scope:string;setScope:(value:string)=>void;from:string;to:string}){
 ...
 return <main className="mx-auto max-w-7xl space-y-5 p-5 text-slate-900 dark:text-slate-100">
  <nav aria-label="Timekeeping sections" className="flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-white p-2 dark:border-slate-700 dark:bg-slate-800"><Link className="rounded-lg px-4 py-3 font-semibold text-violet-700 dark:text-violet-300" to={`/payroll/timekeeping?week=${from}&source=payroll&businessUnit=${encodeURIComponent(unit)}`}>Schedules</Link><span aria-current="page" className="rounded-lg bg-violet-600 px-4 py-3 font-semibold text-white">Import Attendance</span></nav>

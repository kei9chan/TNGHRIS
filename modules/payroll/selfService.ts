...
export type PayLine={deferred?:string;projectedBalance?:string;kind?:string;category?:string;settled?:string;label?:string;amount?:string;remaining?:string;date?:string;quantity?:string;rate?:string;factor?:string;start?:string;end?:string;account?:string;sourceRef?:string;monthly?:string;prior?:string;installment?:string;totalInstallments?:string};
export type EmployeePayslip={id:string;runId:string;employeeName:string;employeeNumber?:string;businessUnit?:string;department?:string;position?:string;from:string;to:string;payDate:string;version:string;payrollStatus:string;paymentStatus?:string;unpaid?:string;releasedAt:string;generatedAt?:string;gross:string;deductions:string;net:string;tax:string;employer?:string;contact:string;lines:PayLine[]|null;contributions:PayLine[]|null;loans:PayLine[]|null;otherDeductions:PayLine[]|null;items:{key:string;label:string}[];correctionKind?:string;originalPayslipId?:string;acknowledgedAt?:string;attendanceSummary?:Record<string,string|number|null|undefined>};
...
export const acknowledgePayslip=(id:string)=>payrollRpc<string>('acknowledge_payroll_payslip',{p_id:id});
